...
Even if you don't care about the startup performance of your app, `MapDataReader` is still 5-7% faster than `Dapper` (note - we're not even using Dapper's command-cache store, just the datareader parser in this bench, actual real world Dapper use will be even slower)
